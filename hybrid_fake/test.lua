...
local target2 = torch.FloatTensor(opt.batchSize,3,opt.fineSize,opt.fineSize)

print('checkpoints_dir', opt.checkpoints_dir)
local netG = util.load(paths.concat(opt.checkpoints_dir, opt.netG_name .. '.t7'), opt)
local netG2 = util.load(paths.concat(opt.checkpoints_dir, opt.netG_name .. '_2.t7'), opt)
--netG:evaluate()
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    paths.mkdir(paths.concat(image_dir,'output2'))
    paths.mkdir(paths.concat(image_dir,'target'))
    paths.mkdir(paths.concat(image_dir,'target2'))
    -- print(input:size())
    -- print(output:size())
    -- print(target:size())
    for i=1, opt.batchSize do
        ...
        image.save(paths.concat(image_dir,'output2',filepaths_curr[i]), image.scale(output2[i],output2[i]:size(2),output2[i]:size(3)/opt.aspect_ratio))
        ...
        image.save(paths.concat(image_dir,'target2',filepaths_curr[i]), image.scale(target2[i],target2[i]:size(2),target2[i]:size(3)/opt.aspect_ratio))
    end
    ...
    if opt.display then
      if opt.preprocess == 'regular' then
        ...
        disp.image(util.scaleBatch(output2,100,100),{win=opt.display_id+2, title='output2'})
        disp.image(util.scaleBatch(target,100,100),{win=opt.display_id+3, title='target'})
        disp.image(util.scaleBatch(target2,100,100),{win=opt.display_id+4, title='target2'})
...
io.write('<tr><td>Image #</td><td>Input</td><td>Output</td><td>Ground Truth</td><td>Output2</td><td>Ground Truth2</td></tr>')
for i=1, #filepaths do
    ...
    io.write('<td><img src="./images/output2/' .. filepaths[i] .. '"/></td>')
    io.write('<td><img src="./images/target2/' .. filepaths[i] .. '"/></td>')
    io.write('</tr>')
end
