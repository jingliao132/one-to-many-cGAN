...
local output_nc2 = opt.output_nc2
...
if opt.which_direction=='AtoB' then
    ...
    idx_B2 = {input_nc+output_nc+1, input_nc+output_nc+output_nc2}
    --print('idx_A, idx_B, idx_B2 :', idx_A, idx_B, idx_B2 )
...
if opt.continue_train == 1 then
   ...
   netG2 = util.load(paths.concat(opt.checkpoints_dir, opt.name, 'latest_net_G_2.t7'), opt)
  ...
  netG2 = defineG(input_nc, output_nc2, ngf)
...
local criterionAE2 = nn.AbsCriterion()
---------------------------------------------------------------------------
optimStateG = {
   learningRate = opt.lr,
   beta1 = opt.beta1,
...
local real_B2 = torch.Tensor(opt.batchSize, output_nc2, opt.fineSize, opt.fineSize)
local fake_B = torch.Tensor(opt.batchSize, output_nc, opt.fineSize, opt.fineSize)
local fake_B2 = torch.Tensor(opt.batchSize, output_nc2, opt.fineSize, opt.fineSize)
local real_AB = torch.Tensor(opt.batchSize, output_nc + input_nc*opt.condition_GAN, opt.fineSize, opt.fineSize)
local real_AB2 = torch.Tensor(opt.batchSize, output_nc2 + input_nc*opt.condition_GAN, opt.fineSize, opt.fineSize)
local fake_AB = torch.Tensor(opt.batchSize, output_nc + input_nc*opt.condition_GAN, opt.fineSize, opt.fineSize)
local fake_AB2 = torch.Tensor(opt.batchSize, output_nc2 + input_nc*opt.condition_GAN, opt.fineSize, opt.fineSize)
...
if opt.gpu > 0 then
   ...
   real_B2 = real_B2:cuda(); fake_B2 = fake_B2:cuda();
   real_AB = real_AB:cuda(); fake_AB = fake_AB:cuda();
   real_AB2 = real_AB2:cuda(); fake_AB2 = fake_AB:cuda();
   if opt.cudnn==1 then
      netG = util.cudnn(netG); netD = util.cudnn(netD);
      netG2 = util.cudnn(netG2);
   end
   netD:cuda(); netG:cuda(); netG2:cuda();
   criterion:cuda(); criterionAE:cuda(); criterionAE2:cuda();
   print('done')
else
...
--    print('real_data size :', real_data:size())
    real_A:copy(real_data[{ {}, idx_A, {}, {} }])   -- img_index, idx_A, w, h
    real_B:copy(real_data[{ {}, idx_B, {}, {} }])   -- img_index, idx_B, w, h
    real_B2:copy(real_data[{ {}, idx_B2, {}, {}}])

    if opt.condition_GAN==1 then
        real_AB = torch.cat(real_A,real_B,2)
        real_AB2 = torch.cat(real_A,real_B2,2)
    else
    ...
    fake_B2 = netG2:forward(real_A)

    if opt.condition_GAN==1 then
        fake_AB = torch.cat(real_A,fake_B,2)
        fake_AB2 = torch.cat(real_A,fake_B2,2);
    else
...
local fDx = function(x)
    ...
    local output2 = netD:forward(real_AB2)
--    local weighted_real_AB = (real_AB + real_AB2)/2
--    local output = netD:forward(weighted_real_AB)
    ...
    local errD_real2 = criterion:forward(output2, label)
    local df_do = criterion:backward(output, label)
    local df_do2 = criterion:backward(output2, label)

    netD:backward(real_AB, df_do)
    netD:backward(real_AB2, df_do2)     -- cascade learning
--    netD:backward(weighted_real_AB, df_do)

    -- Fake
    local output = netD:forward(fake_AB)
    local output2 = netD:forward(fake_AB2)
    netD.output = output
    netD.output2 = output2
--    local weighted_fake_AB = (fake_AB + fake_AB2)/2
--    local output = netD:forward(weighted_fake_AB)
    label:fill(fake_label)
    local errD_fake = criterion:forward(output, label)
    local errD_fake2 = criterion:forward(output2, label)
    local df_do = criterion:backward(output, label)
    local df_do2 = criterion:backward(output2, label)

    netD:backward(fake_AB, df_do)
    netD:backward(fake_AB2, df_do2)     -- cascade learning
--    netD:backward(weighted_fake_AB, df_do)
    
--    errD = (errD_real + errD_fake)/2
    errD = ( (errD_real + errD_real2)/2 + (errD_fake + errD_fake2)/2 )/2
...
-- create closure to evaluate f(X) and df/dX of generator2
local fG2x = function(x)
    netD:apply(function(m) if torch.type(m):find('Convolution') then m.bias:zero() end end)
    netG2:apply(function(m) if torch.type(m):find('Concolution') then m.bias:zero() end end)

    gradParametersG2:zero()

    -- GAN loss
    local df_dg2 = torch.zeros(fake_B2:size())
    if opt.gpu>0 then
       df_dg2 = df_dg2:cuda();
    end

    if opt.use_GAN==1 then
--       local output2 = netD.output -- netD:forward{input_A,input_B} was already executed in fDx, so save computation
       local output2 = netD.output2 -- netD:forward{input_A,input_B} was already executed in fDx, so save computation
       local label = torch.FloatTensor(output2:size()):fill(real_label) -- fake labels are real for generator cost
       if opt.gpu>0 then
          label = label:cuda();
       end
       errG2 = criterion:forward(output2, label)
       local df_do = criterion:backward(output2, label)
       df_dg2 = netD:updateGradInput(fake_AB2, df_do):narrow(2,fake_AB2:size(2)-output_nc2+1, output_nc2)
    else
       errG = 0
    end

    -- unary loss
    local df_do_AE2 = torch.zeros(fake_B2:size())
    if opt.gpu>0 then
       df_do_AE2 = df_do_AE2:cuda();
    end
    if opt.use_L1==1 then
       errL1 = criterionAE2:forward(fake_B2, real_B2)
       df_do_AE2 = criterionAE2:backward(fake_B2, real_B2)
    else
       errL1 = 0
    end

    netG2:backward(real_A, df_dg2 + df_do_AE2:mul(opt.lambda))
...
for epoch = 1, opt.niter do
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
        ...
        -- load a batch and run G and G2 on that batch
        createRealFake()
        ...
        optim.adam(fG2x, parametersG2, optimStateG)
        ...
        if counter % opt.display_freq == 0 and opt.display then
            ...
            if opt.preprocess == 'colorization' then 
                ...
                disp.image(util.deprocess_batch(util.scaleBatch(fake_B2:float(),100,100)), {win=opt.display_id+2, title=opt.name .. ' output2'})
                disp.image(util.deprocess_batch(util.scaleBatch(real_B:float(),100,100)), {win=opt.display_id+3, title=opt.name .. ' target'})
                disp.image(util.deprocess_batch(util.scaleBatch(real_B2:float(),100,100)), {win=opt.display_id+4, title=opt.name .. ' target2'})
            end
        end
                ...
                if opt.preprocess == 'colorization' then 
                    ...
                    for i2=1, fake_B:size(1) do
                        if image_out==nil then
                            image_out = torch.cat(util.deprocess(real_A[i2]:float()),util.deprocess(fake_B[i2]:float()),util.deprocess(fake_B[i2]:float()),3)
                            image_out2 = torch.cat(util.deprocess(real_A[i2]:float()),util.deprocess(fake_B[i2]:float()),util.deprocess(fake_B2[i2]:float()),3)
                        else
                            image_out = torch.cat(image_out, torch.cat(util.deprocess(real_A[i2]:float()),util.deprocess(fake_B[i2]:float()), 3), 2)
                            image_out2 = torch.cat(image_out2, torch.cat(util.deprocess(real_A[i2]:float()),util.deprocess(fake_B2[i2]:float()), 3), 2)
                        end
                    end
                end
            end
            image.save(paths.concat(opt.checkpoints_dir, opt.name , counter .. '_train_res.png'), image_out)
            image.save(paths.concat(opt.checkpoints_dir, opt.name , counter .. '_train_res_2.png'), image_out2)
        ...
        if counter % opt.save_latest_freq == 0 then
            ...
            torch.save(paths.concat(opt.checkpoints_dir, opt.name, 'latest_net_G_2.t7'), netG2:clearState())
    ...
    if epoch % opt.save_epoch_freq == 0 then
        ...
        torch.save(paths.concat(opt.checkpoints_dir, opt.name, epoch .. '_net_G_2.t7'), netG2:clearState())
