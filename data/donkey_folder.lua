...
--   print(img:size())
  ...
  assert(imB2:min()>=-1,"B2: badly scaled inputs")
  ...
  if iH ~= oH or iW ~= oW then 
    ...
    imB2 = image.crop(imB2, w1, h1, w1 + oW, h1 + oH)
  end
  
  if opt.flip == 1 and torch.uniform() > 0.5 then 
    imA = image.hflip(imA)
    imB = image.hflip(imB)
    imB2 = image.hflip(imB2)
  end
...
--    os.exit()
    ...
    --print('In func: loadImageChannel, image A size')
    --print(imA:size())
    --print('In func: loadImageChannel, image B size')
    --print(imB:size())
...
local function loadImage(path)
   ...
   local imB2 = image.crop(input, 2*w/3, 0, w, h)
   --print(imA:size())
   --print(imB:size())
   --print(imB2:size())
   return imA, imB, imB2
...
--     print('process regular')
     local imA, imB, imB2 = loadImage(path)
     imA, imB, imB2 = preprocessAandB(imA, imB, imB2)
     imAB = torch.cat(torch.cat(imA, imB, 1), imB2, 1)
   end
...
trainLoader = dataLoader{
    ...
    sampleSize = {input_nc+output_nc+output_nc2, sampleSize[2], sampleSize[2]},
    split = 100,
